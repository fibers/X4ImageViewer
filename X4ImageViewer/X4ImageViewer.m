...
- (instancetype)initWithFrame:(CGRect)frame images:(NSArray *)images imagePosition:(CGRect)imagePosition withPlaceholder:(UIImage *)placeholderImage{
    ...
    if(self){
        ...
        UIView *paginationContainer = [[UIView alloc] initWithFrame:rectPaginationContainer];
        [self addSubview:paginationContainer];
        ...
        [paginationContainer addSubview:self.pageControlPagination];
        ...
        [paginationContainer addSubview:self.numberPagination];
        
        if( [[images firstObject] isKindOfClass:[NSURL class]]){
    ...
    if(self.placeholderImage){
        if([ivImage.image isEqual:self.placeholderImage]){
            ...
            if(self.bWebImageURL){
                [ivImage sd_setImageWithURL:[self.images objectAtIndex:index]];
            }else{
